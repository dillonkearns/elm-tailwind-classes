...
export function elmTailwind(options = {}) {
  ...
  return {
    ...
    async buildStart() {
      ...
      // Step 2: Run elm-review extraction for initial class list.
      // This runs once at startup (~5s). During HMR, we skip re-extraction —
      // @source inline() covers parameterized classes, and @tailwindcss/vite
      // scans the compiled JS for static classes.
      ...
      // Write extracted classes to safelist for both dev and production
      ...
      if (!isBuild) {
        // Dev: also add @source inline() for all parameterized class combinations.
        // This covers NEW classes added during HMR without needing re-extraction.
        const sourceInlines = codegenResult?.sourceInlines || [];
        for (const line of sourceInlines) {
          wrapperCss += `${line}\n`;
        }
        log(`Wrote ${sourceInlines.length} @source inline() directives for dev HMR`);
      }
