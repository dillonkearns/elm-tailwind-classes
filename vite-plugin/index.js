...
export function elmTailwind(options = {}) {
  ...
  let isBuild = false;     // true for production builds, false for dev server
  ...
  // Run code generation. Returns { success, sourceInlines } or false on failure.
  ...
  return {
    ...
    async buildStart() {
      ...
      const codegenResult = await runCodegen(cwd);
      codegenComplete = !!codegenResult;
      ...
      // Step 2: For production builds, run elm-review extraction for precise class list.
      // For dev, use @source inline() directives instead (instant, no elm-review needed).
      if (isBuild) {
        cleanElmReviewTemplates(cwd);
        extractedClasses = runExtraction(cwd);
      } else {
        log('Dev mode: skipping elm-review extraction, using @source inline() instead');
      }

      // Step 3: Generate wrapper CSS
      userCssFile = findCssPath(cwd);
      const generatedStylesPath = path.join(cwd, generateDir, 'styles.css');
      const elmTailwindDir = path.join(cwd, generateDir);

      let wrapperCss = `/* Generated by elm-tailwind-classes - do not edit */\n`;

      if (isBuild) {
        // Production: use extracted class list for precise CSS output
        ...
        wrapperCss += `@source "./safelist.txt";\n`;
      } else {
        // Dev: use @source inline() for all parameterized class combinations.
        // This tells Tailwind about classes that use string concatenation in
        // compiled JS (e.g. "p-" ++ "4") without needing elm-review extraction.
        const sourceInlines = codegenResult?.sourceInlines || [];
        for (const line of sourceInlines) {
          wrapperCss += `${line}\n`;
        }
        log(`Wrote ${sourceInlines.length} @source inline() directives for dev`);
      }

      if (userCssFile) {
        const relativeToWrapper = path.relative(elmTailwindDir, userCssFile);
        wrapperCss += `\n@import "${relativeToWrapper}";\n`;
        log('Wrapper will import original CSS at', relativeToWrapper);
      } else {
        wrapperCss = `/* Generated by elm-tailwind-classes - do not edit */\n@import "tailwindcss";\n` +
          wrapperCss.replace(`/* Generated by elm-tailwind-classes - do not edit */\n`, '');
      }
    ...
    resolveId(id, importer) {
      ...
      // Don't redirect imports from our own generated wrapper (avoid circular redirect)
      if (importer && path.resolve(importer).startsWith(path.resolve(process.cwd(), generateDir))) {
        return null;
      }
    ...
    // HMR: log Elm file changes (no extraction needed in dev — @source inline covers it)
    handleHotUpdate({ file }) {
      if (!file.endsWith('.elm')) return;

      // Ignore generated files in .elm-tailwind/ — these are our own output
      const genDir = path.resolve(process.cwd(), generateDir);
      if (file.startsWith(genDir)) return;

      log('Elm file changed:', path.basename(file));
      // No extraction needed — @source inline() directives in the wrapper CSS
      // already tell Tailwind about all possible parameterized class combinations.
      // @tailwindcss/vite handles CSS HMR when it detects new classes in the
      // compiled JS output from vite-plugin-elm.
    }
  };
}
