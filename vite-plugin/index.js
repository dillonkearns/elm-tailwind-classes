...
export function elmTailwind(options = {}) {
  ...
  return {
    ...
    async buildStart() {
      ...
      if (userCssFile) {
        // Import the original CSS file directly (not a copy) so that relative
        // paths (@import, url(), etc.) resolve from the original location.
        ...
        // No user CSS found — include Tailwind directly in the wrapper
    ...
    handleHotUpdate({ file, server }) {
      ...
      extractTimeout = setTimeout(async () => {
        ...
        if (JSON.stringify(newClasses) !== JSON.stringify(extractedClasses)) {
          ...
          // Update wrapper CSS with inlined classes to trigger Vite's CSS HMR.
          // We use @source inline() instead of writing safelist.txt because
          // file writes to non-CSS assets trigger full page reloads in Vite.
          const wrapperPath = path.join(cwd, generateDir, 'styles.css');
          let wrapperContent = fs.readFileSync(wrapperPath, 'utf-8');
          // Replace the @source line (file or inline) with updated inline classes
          wrapperContent = wrapperContent.replace(
            /@source [^\n]+\n/,
            `@source inline("${extractedClasses.join(' ')}");\n`
          );
          // Update timestamp to ensure Vite sees a change
          wrapperContent = wrapperContent.replace(
            /\/\* Generated by elm-tailwind-classes[^*]*\*\//,
            `/* Generated by elm-tailwind-classes - ${Date.now()} */`
          );
          fs.writeFileSync(wrapperPath, wrapperContent);
          log('Updated wrapper CSS with inline classes');

          // Notify Vite's watcher about the CSS change
          setTimeout(() => {
